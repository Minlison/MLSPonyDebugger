...
def after_install(options, home_dir):
    ...
    os.chdir(upper_path)
    # install pybonjour
    os.chdir(upper_path + "/pybonjour-1.1.1")
    subprocess.check_call([join(home_dir, 'bin', 'python'),
                        'setup.py', 'install'])
...
def main():
    ...
    parser.add_option(
        "--ponyd-symlink",
        metavar="PONYD_SYMLINK_PATH", 
        help="Optional location to symlink ponyd to. /usr/local/bin/ponyd is recommended"
    )
